...
var root = {
  getProducts: async () => {
    return await query(
      `select ID, Name, Price, Cost, Description, Img, Update_time from product_list_tbl;`
    )
      .then((rtn) => {
        return {
          res: 0,
  ...
  addProduct: async (productInfo) => {
    ...
    let sql = `insert into product_list_tbl (Name,Price,Cost,Description,Img,Update_time) values (?,?,?,?,?,'${timeFormart(
      new Date()
    )}');`;
    let param = [
      Name,
  ...
  modifyProduct: async ({ ID, Name, Price, Cost, Description, Img }) => {
    return await query(`update product_list_tbl 
            set Name='${Name || "Name"}',Price=${Price || "Price"},Cost=${
      Cost || "Cost"
    },Description='${Description || "Description"}',Update_time='${timeFormart(
      new Date()
    )}'
            where ID=${ID};`)
      .then((rtn) => {
