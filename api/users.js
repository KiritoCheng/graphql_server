...
var root = {
  getUsers: async ({ Uid }) => {
    return await query(
      `select Uid,Username,Password,Headimg,Mail from users where Uid=${Uid};`
    )
      .then((rtn) => {
        return {
          res: 0,
  ...
  addUser: async ({ Username, Password, Mail }) => {
    let count = await query(
      `select count(1) as count from users where Mail='${Mail}'`
    ...
    return await query(
      ...
      .catch((err) => {
        return { res: -1, errors: err, msg: "" };
      });
  },
  modifyUser: async ({ Uid, Username, Password, Headimg, Mail }) => {
    return await query(
      `update users set ${updateSql([
        Username,
        Password,
        Headimg,
        Mail,
